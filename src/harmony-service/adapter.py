"""`HarmonyAdapter` for Harmony Vector Flow Service (harmony-flow).

The class in this file is the top level of abstraction for a service that
accepts netCDF/HDF5 or GeoTIFF and create a PNG image texture encoding u/v
vector components, along with an ESRI worldfile for georeferencing.
"""
...
class VectorFlowAdapter(BaseHarmonyAdapter):
    """Vector Flow Service extension to the harmony-service-lib BaseHarmonyAdapter."""

    def invoke(self) -> tuple[HarmonyMessage, Catalog | list]:
    ...
    def validate_message(self) -> None:
        ...
        if has_scale_extents(self.message) or has_scale_sizes(self.message):
            if not has_crs(self.message):
                raise VectorFlowInvalidMessageError(
    ...
    def process_item(self, item: Item, source: HarmonySource) -> Item:
        ...
        with TemporaryDirectory() as working_directory:
            try:
                ...
                raise VectorFlowServiceError(str(exception)) from exception
